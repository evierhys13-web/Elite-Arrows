...
export function AuthProvider({ children }) {
  ...
  // Auto-expire confirmed fixtures after 24 hours without a result
  useEffect(() => {
    if (!user?.id || !fixtures.length) return

    const checkExpiredFixtures = async () => {
      const now = Date.now()
      const dayInMs = 24 * 60 * 60 * 1000

      const expiredFixtures = fixtures.filter(f => {
        // Only check confirmed fixtures the current user is part of (to ensure permission to update)
        const isParticipant = String(f.player1Id || f.player1) === String(user.id) || String(f.player2Id || f.player2) === String(user.id)
        if (!isParticipant && !user.isAdmin) return false

        if (String(f.status).toLowerCase() !== 'accepted') return false

        // Get start time
        const fDate = f.fixtureDate || f.date || f.proposedDate || ''
        const fTime = f.fixtureTime || f.time || f.proposedTime || ''
        if (!fDate) return false

        // Attempt to parse date robustly
        let startTime = 0
        try {
          startTime = new Date(`${fDate}T${fTime || '00:00'}`).getTime()
        } catch (e) { return false }

        if (!startTime || Number.isNaN(startTime)) return false

        // Check if 24h has passed
        if (now - startTime < dayInMs) return false

        // Check if a result exists in results list
        const hasResult = results.some(r => {
           if (['pending', 'approved'].includes(String(r.status).toLowerCase())) {
             if (String(r.fixtureId) === String(f.id)) return true
             if (f.cupId && r.cupId && String(r.cupId) === String(f.cupId) && String(r.matchId) === String(f.matchId)) return true

             // Basic player match fallback for results without fixtureId link
             const p1 = f.player1Id || f.player1
             const p2 = f.player2Id || f.player2
             const playersMatch = (String(r.player1Id) === String(p1) && String(r.player2Id) === String(p2)) ||
                                  (String(r.player1Id) === String(p2) && String(r.player2Id) === String(p1))
             if (playersMatch && !r.fixtureId) {
                const rType = String(r.gameType || '').toLowerCase()
                const fType = String(f.gameType || '').toLowerCase()
                if (rType === fType || (fType === 'league' && rType === 'playoff')) return true
             }
           }
           return false
        })
      ...
      if (expiredFixtures.length > 0) {
        for (const f of expiredFixtures) {
          try {
            console.log('Expiring fixture:', f.id)
            const updates = {
              status: 'pending',
              ...
              counterBy: null,
              updatedAt: new Date().toISOString(),
              expirationLog: `Auto-expired 24h after ${f.fixtureDate} ${f.fixtureTime}`
            }

            await updateDoc(doc(db, 'fixtures', String(f.id)), updates)

            // Notify participants
            const p1 = f.player1Id || f.player1
            const p2 = f.player2Id || f.player2
            const otherId = String(p1) === String(user.id) ? p2 : p1

            if (otherId) {
              await notifyUser(
                otherId,
                'Fixture Expired',
                `Your confirmed match against ${user.username} has expired as no result was submitted within 24 hours. It is now unarranged.`,
                'fixture_expired',
                { fixtureId: f.id }
              )
            }
          } catch (e) {
            console.error('Error expiring fixture:', f.id, e)
          }
        }
    ...
    const timer = setTimeout(checkExpiredFixtures, 3000)
    return () => clearTimeout(timer)
  }, [fixtures, results, user?.id, user?.isAdmin])
