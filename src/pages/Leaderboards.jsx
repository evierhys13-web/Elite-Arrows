...
export default function Leaderboards() {
  ...
  const rankColor = (i) => i === 0 ? '#fbbf24' : i === 1 ? '#94a3b8' : i === 2 ? '#d97706' : 'inherit'

  const renderPlayerCell = (player) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => navigate(`/statistics/${player.id}`)}>
      <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--accent-primary)', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px' }}>
          {['week', 'month', 'quarter', 'all'].map(f => (
            <button key={f} className={`btn btn-sm ${timeFilter === f ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTimeFilter(f)} style={{ fontSize: '0.7rem', padding: '6px 12px' }}>
            ...
            <tbody>
              {leagueBoard.map((player, i) => (
                <tr key={player.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', fontSize: '0.9rem' }}>
                  <td style={{ padding: '12px 8px', fontWeight: 900, color: rankColor(i) }}>#{i + 1}</td>
                  <td style={{ padding: '12px 8px' }}>{renderPlayerCell(player)}</td>
  ...
  const render180sTable = () => (
    <>
      <div style={{ marginBottom: '20px' }}>
        <h3 className="card-title" style={{ margin: 0 }}>Most 180s</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '6px', marginBottom: 0 }}>All-time 180s across all league games</p>
      </div>
      <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {board180s.length === 0 ? (
  ...
  const renderCheckoutsTable = () => (
    <>
      <div style={{ marginBottom: '20px' }}>
        <h3 className="card-title" style={{ margin: 0 }}>Highest Checkouts</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '6px', marginBottom: 0 }}>Best single checkouts across all seasons</p>
      </div>
      <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
            ...
            <tbody>
              {boardCheckouts.map((player, i) => (
                <tr key={player.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', fontSize: '0.9rem' }}>
                  <td style={{ padding: '12px 8px', fontWeight: 900, color: rankColor(i) }}>#{i + 1}</td>
                  <td style={{ padding: '12px 8px' }}>{renderPlayerCell(player)}</td>
                  <td style={{ padding: '12px 8px', textAlign: 'center', fontWeight: 900, color: 'var(--accent-cyan)' }}>{player.highestCheckout}</td>
                  <td style={{ padding: '12px 8px', textAlign: 'center', color: 'var(--text-muted)' }}>{player.played}</td>
                  <td style={{ padding: '12px 8px', textAlign: 'center', fontWeight: 700, color: player['180s'] > 0 ? '#fbbf24' : 'var(--text-muted)' }}>
                    {player['180s'] || '-'}
                  </td>
              ...
              {practiceLeaderboard.map((entry, index) => (
                <tr key={entry.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', fontSize: '0.9rem' }}>
                  ...
                  <td style={{ padding: '12px 8px' }}>
                    <span style={{ padding: '4px 8px', background: 'rgba(255,255,255,0.05)', borderRadius: '4px', fontSize: '0.75rem', color: entry.modeId === 'atc' ? 'var(--accent-cyan)' : entry.modeId === '170' ? '#ef4444' : '#fbbf24' }}>
    ...
    <div className="page animate-fade-in">
      ...
      <div className="division-tabs" style={{ marginBottom: '24px' }}>
        {[
          ...
          { key: 'practice', label: '🎯 Practice Drills' },
        ].map(tab => (
          <button key={tab.key} className={`division-tab ${activeTab === tab.key ? 'active' : ''}`} onClick={() => setActiveTab(tab.key)}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab !== 'practice' && (
        <div className="division-tabs" style={{ marginBottom: '20px', overflowX: 'auto', paddingBottom: '4px' }}>
          {divisions.map(div => (
            <button key={div} className={`division-tab ${selectedDivision === div ? 'active' : ''}`} onClick={() => setSelectedDivision(div)} style={{ fontSize: '0.75rem', padding: '8px 14px' }}>
              {div === 'all' ? 'All' : div}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="card glass" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <h3 className="card-title" style={{ margin: 0 }}>Hall of Fame</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {honoursList.length > 0 ? honoursList.map((honour, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid var(--border)' }}>
